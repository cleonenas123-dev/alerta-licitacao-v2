...
export default function ConfirmClient() {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        // 0) Se o Supabase mandar erro na QUERY (seu print mostra isso!)
        const qError = sp.get("error") || "";
        const qErrorCode = sp.get("error_code") || "";
        const qDesc = sp.get("error_description") || "";

        if (qError || qErrorCode || qDesc) {
          setIsErro(true);

          if (qErrorCode === "otp_expired") {
            setMsg("Este link de confirmação expirou. Volte ao login e solicite um novo e-mail.");
          } else {
            setMsg(
              "Não foi possível confirmar seu e-mail. " +
                (qDesc ? decodeURIComponent(qDesc) : "Tente novamente.")
            );
          }
